...
{
    [ApiVersion("1.0")]
    public class ProjectPlansController : DataController_base
    {
        private readonly ILoggerFactory _loggerFactory;
        public ProjectPlansController(ILoggerFactory loggerFactory)
        {
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] GetProjectPlansQuery filter)
        {
        ...
        {
            return Ok(await Mediator.Send(new GetProjectPlanByIdQuery { Id = id }));
        }
        ...
        [HttpPost]
        // [Authorize]
        public async Task<IActionResult> Post(CreateProjectPlanCommand command)
        {
        ...
        [Route("AddMock")]
        // [Authorize]
        public async Task<IActionResult> AddMock(InsertMockProjectPlanCommand command)
        {
            return Ok(await Mediator.Send(command));
        }
        ...
        [HttpPut("{id}")]
        [Authorize]
        public async Task<IActionResult> Put(Guid id, UpdateProjectPlanCommand command)
        {
            if (id != command.Id)
            {
                return BadRequest();
