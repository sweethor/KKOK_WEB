...
{
    [ApiVersion("1.0")]
    public class MembersController : DataController_base
    {
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] GetMembersQuery filter)
        {
        ...
        {
            return Ok(await Mediator.Send(new GetMemberByIdQuery { Id = id }));
        }
        ...
        [Route("AddMock")]
        // [Authorize]
        public async Task<IActionResult> AddMock(InsertMockMemberCommand command)
        {
            return Ok(await Mediator.Send(command));
        }
        ...
        [HttpPut("{id}")]
        [Authorize]
        public async Task<IActionResult> Put(Guid id, UpdateMemberCommand command)
        {
            if (id != command.Id)
            {
                return BadRequest();
